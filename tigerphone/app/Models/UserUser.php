...
//        echo $sql;
    ...
    public function getFormattedQqAttribute(){
     ...
     */
    public static function getRegistCode($sCode = null, & $oUserRegisterLink) {
        ...
        if (!$sCode) {
            ...
            // 模拟oPrizeGroup对象
            $oUserRegisterLink = $oExpirenceAgent;
            $oUserRegisterLink->is_admin = 0;
            $oUserRegisterLink->is_agent = 1;
            $oUserRegisterLink->user_id = $oExpirenceAgent->id;
        } else {
            $oUserRegisterLink = UserRegisterLink::getRegisterLinkByPrizeKeyword($sCode);

            // TODO 此处注册失败的具体条件后续可以改进
            if (!$oUserRegisterLink) {
                return false;
            }
            // 总代开户链接只能使用一次
            if ($oUserRegisterLink->is_admin && $oUserRegisterLink->created_count) {
                return false;
            }
        }
        return true;
