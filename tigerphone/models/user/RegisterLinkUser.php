...
class RegisterLinkUser extends BaseModel {
    ...
    public function getFirstDepositAtAttribute(){
        $oUser = User::find($this->user_id);
        return date('Y-m-d', strtotime($oUser->first_deposit_at));
    }
    public function getFirstDepositAmountAttribute(){
        $oUser = User::find($this->user_id);
        return $oUser->first_deposit_amount;
    }
}